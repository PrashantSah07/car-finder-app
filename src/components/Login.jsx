...
const Login = ({ showLogin, setShowLogin, setIsShow }) => {
    ...
    return (
        <>
            <motion.div className='h-[100vh] w-[100vw]  fixed top-0 bg-[#0000007b] z-80 flex justify-center items-center' onClick={function () {
                setShowLogin(false)
                setErrors({ firstName: '', emailNumber: '', })
                ...
                <div className={`w-[400px] flex flex-col gap-10 p-5 rounded-2xl mt-10 mx-5 ${darkTheme ? 'bg-[#1f2937]' : 'bg-white'}`} onClick={function (e) { e.stopPropagation(); }}>
                    <div className='flex items-center justify-between'>
                        ...
                        <button onClick={function () {
                            setShowLogin(false)
                            setErrors({ firstName: '', emailNumber: '', })
                        }} className='p-1 rounded-full cursor-pointer text-white hover:bg-gray-200'><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="rgba(0,0,0,1)"><path d="M11.9997 10.5865L16.9495 5.63672L18.3637 7.05093L13.4139 12.0007L18.3637 16.9504L16.9495 18.3646L11.9997 13.4149L7.04996 18.3646L5.63574 16.9504L10.5855 12.0007L5.63574 7.05093L7.04996 5.63672L11.9997 10.5865Z"></path>
                            </svg></button>
                    </div>
                    <form onSubmit={handleSubmit} className='flex flex-col gap-5'>
                        ...
                        <div>
                            <input className='focus:outline-none border-1 w-full p-2 border-[#d3caca] rounded-md' type="text" name="emailNumber" placeholder='Email or Number' onChange={handleChange} />
                            {errors.emailNumber && <p className='text-[13px] pl-2 text-red-500'>{errors.emailNumber}</p>}
                        </div>
                        <button disabled={loading} className='w-full p-2 cursor-pointer bg-blue-600 text-white rounded-md hover:bg-blue-700 transition' type='submit' value='login'>{loading ? 'Please Wait' : 'Login'}</button>
